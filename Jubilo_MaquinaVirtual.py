...
def ejecucion():
	...
	while noTermina:
		...
		#Asignacion
			...
			fillValor(currentEjecucion, quad[3], getType(quad[3]), auxValor)
		...
		#Sort & Transpose
			...
			'''
		elif quad[0] == 'plothist':
			memPosBase = int(quad[1])
			columnas = int(quad[2])
			auxBins = getValor(currentEjecucion, int(quad[3]), getType(int(quad[3])))
			auxBins = int(auxBins)
			auxArray = []
			for x in range(columnas - 1):
				auxArray.append(getValor(currentEjecucion, memPosBase+x, getType(memPosBase+x)))
			...
			plt.show()
			plt.close()
		elif quad[0] == 'plotline':
			columnas = int(quad[3])
			memPosBase1 = int(quad[1])
			memPosBase2 = int(quad[2])
			auxArray1 = []
			auxArray2 = []
			for x in range(columnas -1):
				auxArray1.append(getValor(currentEjecucion, memPosBase1+x, getType(memPosBase1+x)))
				auxArray2.append(getValor(currentEjecucion, memPosBase2+x, getType(memPosBase2+x)))
			plt.plot(auxArray1, auxArray2)
			plt.show()
			plt.close()
		elif quad[0] == 'lineareg':
			columnas = int(quad[3])
			memPosBase1 = int(quad[1])
			memPosBase2 = int(quad[2])
			auxArray1 = []
			auxArray2 = []
			for x in range(columnas -1):
				a1 = getValor(currentEjecucion, memPosBase1+x, getType(memPosBase1+x))
				a2 = getValor(currentEjecucion, memPosBase2+x, getType(memPosBase2+x))
				auxArray1.append(int(a1))
				auxArray2.append(int(a2))
			X = np.array(auxArray1).reshape(-1 ,1)
			Y = np.array(auxArray2).reshape(-1 ,1)
			...
			plt.close()
		#Export csv
		elif quad[0] == 'exportcsv':
			memPosBase = int(quad[1])
			csvName = str(quad[2])
			sizes = str(quad[3])
			columnas = 0
			renglones = 0
			#print("sizes: ", sizes.find('/'))
			if sizes.find('/') >= 1:
				columnas = sizes[0:sizes.find('/')]
				columnas = int(columnas)
				renglones = sizes[sizes.find('/')+1:]
			...
			for x in range(renglones):
				...
				for y in range(columnas):
					s = memPosBase+y+x*columnas
					auxArray.append(getValor(currentEjecucion, s, getType(s)))
				auxMat.append(auxArray)
			auxDF = pd.DataFrame(auxMat)
			auxDF.to_csv(csvName[1:-1])

		elif quad[0] == 'exchange':
			size = str(quad[3])
			memPosBase1 = int(quad[1])
			memPosBase2 = int(quad[2])
			if size == 'single': #Solo se cambian las variables
				aux1 = getValor(currentEjecucion, memPosBase1, getType(memPosBase1))
				aux2 = getValor(currentEjecucion, memPosBase2, getType(memPosBase2))
				fillValor(currentEjecucion, memPosBase1, getType(memPosBase1), aux2)
				fillValor(currentEjecucion, memPosBase2, getType(memPosBase2), aux1)
			else: #Size es un arreglo o mtriz
				columnas = sizes[0:sizes.find('/')]
				columnas = int(columnas)
				renglones = sizes[sizes.find('/')+1:]
				renglones = int(renglones)
